	...
	function parse(tokens, strictSemicolons) {
		...
		parseProcs= {
			...
			"function": function(functionToken, literal) {
				...
				var bodyNode = new asn.BlockNode(blockLeftBrace, body,  expectPunc("}"));
				return new (literal ? asn.FunctionLiteralNode : asn.FunctionDefNode)(functionToken, nameToken, lambdaLeftParen, parameterList, lambdaRightParen, bodyNode);
			},
		...
		objectExpr= function() {
			...
			list= [];
			while (!token.puncEq("}")) {
				list.push(new asn.PropertyNode(getName(), expectPunc(":"), expression(false), token.puncEq(",") && next()));
			}
		...
		subscriptsExpr= function(expr, allowCalls) {
			...
			if (token.puncEq(".")) {
				...
				return subscriptsExpr(new asn.ApplicationNode(expr, opening, args, closing), true);
		...
		isAssignable= function(expr) {
			return expr.type===asn.NameNode || expr.type===asn.SubscriptNode || (expr.type===asn.BinaryOpNode && expr.opToken.eq(tPunc, "."));
		},
