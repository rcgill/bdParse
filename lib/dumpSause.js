define(function(){
	...
	var sause = {
		...
		beforeUnaryPrefix:function(){
			writeln("unary prefix op = " + this.opToken.value);
		},

		beforeUnaryPostfix:function(){
			writeln("unary postfix op = " + this.opToken.value);
		},

		BinaryOp:function(sause){
			writeln("binary op:" + this.opToken.value);
			spaceCount += 2;
			writeln("lhs");
		...
		Object:function(sause){
			...
			this.propertyList.forEach(function(item){
				writeln(item.nameToken.value);
				spaceCount += 2;
				traverse(item.initialValue, sause);
				spaceCount -= 2;
			});
