define(["./types", "./tokenize", "./parse", "./asn"], function(types, tokenize, parse, asn) {
	///
	...
	var
	    ...
	    filterComments= function(
			...
			return tokens.filter(function(item) { return item.type!==tLineComment && item.type!==tBlockComment; });
	    },
	    ...
	    parseText= function(
			...
			//
			var
		        tokens= (filter || filterComments)(tokenize((text= split(text)))),
		        tree= parse(tokens);
			return [text, tree, tokens];
	    };

	return types.mix(types.mix({}, types, asn), {
		tokenize:tokenize,
		parse:parse,
